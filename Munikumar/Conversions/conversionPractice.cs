...
class TypeCastingExamples
{
    static void Main()
    {
        ...
        // it show invalidcastexpection error
        ...
        // it rise a invalidcastexception
        ...
        // it raises an error cannot covert nullinteger to int explicitly
        object boxedValue = "100";//Boxing
        int unboxedInt = (int)boxedValue; //Un-Boxing 
        //it raises an invalidcast exception because in  object it stored as string. so, it cann't convert

        object boxedValue = int.Parse("100");//Boxing
        int unboxedInt = (int)boxedValue;//Un-Boxing
        //It will work it doesn't show invalidcastexception.
    }
}
...
namespace TypeConversionComplexExample1
{
    class Program
    {
        static void Main()
        {
            ...
            if (int.TryParse((withdrawalObj as string, out withdrawalAmount))
            {
            ...
            //o/p: Calculated interset :47.1225
            ...
            //o/p:Simulating Monthly Transactions:
            ...
            try
            {
                string invalidCast = (string)accountId;  // Intentional error: invalid cast exception
            }
            catch (InvalidCastException ex)
            {
                Console.WriteLine("Error: Attempted invalid cast from int to string.");
            }
            //op:Error: Attempted invalid cast from int to string.
